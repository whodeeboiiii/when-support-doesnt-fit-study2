"""dossier **researcher_only** 층 로더 — 연구자 콘솔 전용 (구현명세서 §1.2 · §5.3).

이 모듈이 존재하는 이유는 하나다: researcher_only(회고 stance·미전송 생각·mismatch 해석·
원 trajectory·ideal response·correction labor)는 §1.2 표에서 **AI2·checker 전부 금지**이고, 금지를 주석이 아니라 **모듈 경계**로 만들기 위해서다.
...
#: §5.3 스키마의 researcher_only 필드. 없는 키는 빈 문자열로 채워 콘솔 표시를 단순화한다.
...
def load_researcher_only(participant_no: str) -> dict[str, Any]:
    ...
    ai_visible·stimulus·evidence_code는 여기서 돌려주지 않는다 — 필요하면 호출부가 `dossier_loader`를
    ...
    if not isinstance(layer, dict):
        raise ValueError(f"{participant_no}: researcher_only 층이 객체가 아니다 (§5.3)")
