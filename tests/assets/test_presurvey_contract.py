"""NT-05 — 사전설문 자산 계약과 메타키 미노출 (v1.0.1 §4.2 · §7.1 — D-44로 복원).
...
(CLAUDE.md 자산 원칙: 자산 착지·변경 커밋은 자산 계약 테스트를 동반한다).

v1.0.1에서 그대로 가져왔고 두 곳만 v2.0 로더에 맞췄다: 자산 경로가 `ASSET_PATH` 단일
상수에서 `_v1`→`_v0` 후보 목록이 됐고(`rating_items` 패턴), 절 번호가 v1.0.1을 가리킨다.
"""
...
def test_contract_violations_fail_loudly(tmp_path, monkeypatch) -> None:
    ...
    (tmp_path / "presurvey_items_v0.json").write_text(
        json.dumps({"version": "x", "items": [{"id": "a", "type": "unknown", "text": "t"}]}),
        encoding="utf-8",
    )
    monkeypatch.setattr(presurvey, "FIXTURES_DIR", tmp_path)
...
def test_missing_asset_is_not_silently_empty(tmp_path, monkeypatch) -> None:
    """`_v1`·`_v0` 어느 쪽도 없으면 기동이 끊긴다 — 문항 0개로 뜨는 화면을 만들지 않는다."""
    monkeypatch.setattr(presurvey, "FIXTURES_DIR", tmp_path)
    presurvey.reset_cache()
    try:
        with pytest.raises(presurvey.PresurveyContractError, match="PH-01"):
            presurvey.load()
    finally:
        presurvey.reset_cache()


def test_v1_wins_over_the_placeholder(tmp_path, monkeypatch) -> None:
    """PH-01 해소 = `_v1`을 놓는 것이다. 로더가 그것을 잡고 `is_placeholder`가 내려간다."""
    source = json.loads(presurvey.asset_path().read_text(encoding="utf-8"))
    for name in ("presurvey_items_v0.json", "presurvey_items_v1.json"):
        (tmp_path / name).write_text(json.dumps(source, ensure_ascii=False), encoding="utf-8")
    monkeypatch.setattr(presurvey, "FIXTURES_DIR", tmp_path)
    presurvey.reset_cache()
    try:
        assert presurvey.asset_path().name == "presurvey_items_v1.json"
        assert presurvey.load().is_placeholder is False
    finally:
        presurvey.reset_cache()


def test_placeholder_state_is_visible_in_the_recruiting_gate() -> None:
    """파일명이 곧 상태 표시다 — `_v0`이면 모집 게이트가 PH-01을 계속 보고한다(D-44)."""
    from app.core import freeze

    asset = presurvey.load()
    assert asset.is_placeholder is (asset.source_path.name.endswith("_v0.json"))
    reported = {blocker.tag for blocker in freeze.blockers()}
    assert ("PH-01" in reported) is asset.is_placeholder
